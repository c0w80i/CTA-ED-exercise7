...
```{r, echo = F, eval = T}
library(kableExtra) #forgot this! 
tweets_sample_kable <- tweets_sample %>% #making variable tweets_sample_kable from tweets_sample and then
  mutate(text = gsub("(@)(\\S)", "\\1 \\2", text), #regex looking for @ followed by anything but white space--basically looking for usernames and replaces them with a space between the @ and the username
         sourcetweet_text = gsub("(@)(\\S)", "\\1 \\2", sourcetweet_text), #doing same thing here
         user_description = gsub("(@)(\\S)", "\\1 \\2", user_description)) #and here

kableExtra::kable(head(tweets_sample_kable), format = "html") #6 first lines in a table using kableExtra package 
```
...
```{r, echo = F, eval = T}
setwd("/Users/sarahb/Downloads/CTA-ED-exercise7-main")
tweets_tox_sample <- readRDS("tweets-tox-ranked.rds")
```

```{r, echo = F, eval = T}
tweets_tox_sample_kable <- tweets_tox_sample %>% #same thing as earlier but with the tox sample data! 
...
kableExtra::kable(head(tweets_tox_sample_kable), format = "html") #data much more processed for me, less work for me, yay!
```
...
tweets_tox_select <- tweets_tox_sample %>% #new df called tweets_tox_select from tweets_tox_sample and then use
  select(tweet_id, user_username, text, TOXICITY) %>% #select to only include tweet_id, user_username (tautological label), text, and toxicity
  mutate(toxbin = ifelse(TOXICITY>=.5, 1, 0)) #adding new column called toxbin--toxicity binary--filling based on toxicity score using ifelse or if then else so if it is greater or equal to 0.5 it is assigned 1 meaning toxic and if it is less than .5 it is 0 or non-toxic

set.seed(123) #ensuring 'random' sample is same for everyone 
split <- initial_split(tweets_tox_select, prop = .7, strata = "TOXICITY") #split into training set using 70% for training ensuring a balance of TOXICITY levels across the training set 
train <- training(split) #taking the 70% and assigning it to train
test  <- testing(split) #taking the rest of the df for testing 

table(train$toxbin) %>% prop.table() #frequency table of toxic versus non-toxic 
table(test$toxbin) %>% prop.table() #same as above
#equal split! 
```
...
# Text preprocessing creating function
preprocess <- function(corpus) {
  corpus <- tm_map(corpus, content_transformer(tolower)) #lowercase
  corpus <- tm_map(corpus, removePunctuation) #removepunct
  corpus <- tm_map(corpus, removeNumbers) #removeNumbers
  corpus <- tm_map(corpus, removeWords, stopwords("english")) #removestopwords
  corpus <- tm_map(corpus, stemDocument) #stem words
  return(corpus)
}

train_corpus <- preprocess(train_corpus) #running function on training set
test_corpus <- preprocess(test_corpus) #running function on testing set
...
trControl <- trainControl(method = "cv", number = 10) #train control from caret creatning rules so using cross validation and split into 10 parts training on 9 parts adn testing on the last one and redoing it 10 times so every part gets tested
...
Great! We have 95% accuracy. That's good, right? WRONG.  CLASS IMBALANCE ! gross. 
...
library(peRspective) #not sure what to do here because i cannot access peRspective ! 
library(dplyr)
library(ggplot2)
